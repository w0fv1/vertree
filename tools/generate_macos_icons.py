...
def main() -> None:
    ...
    # Tune these if the Dock icon looks too big/small.
    # Smaller by ~5% compared to previous (0.76 * 0.95 = 0.722).
    app_icon_scale = 0.722
    tray_icon_scale = 0.62
    # Dock icon background: macOS-style rounded-rect app icon with a white plate.
    app_icon_bg = _parse_hex_color("#FFFFFF")
    ...
    bg_inset = max(1, int(round(size * 0.10)))
    bg_size = size - 2 * bg_inset
    radius = max(1, int(round(bg_size * 0.223)))
    ...
    tray_template = _composite_on_canvas(cropped, 32, tray_icon_scale, monochrome=True)
    ...
    tray_glyph_scale = 0.52
    tray_glyph_mask = _composite_on_canvas(
        cropped,
        32,
        tray_glyph_scale,
        monochrome=True,
        offset_x=1,  # nudge right to better optical centering in the menu bar
    )
    tray_glyph_mask = _dilate_alpha(tray_glyph_mask, radius=1)  # thicken thin strokes
    tray_bg = Image.new("RGBA", (32, 32), (0, 0, 0, 0))
    tray_bg = _add_circle_bg_underlay(
        tray_bg,
        inset=2,
        fill=_parse_hex_color("#FFFFFF"),
    )
